...
**Date :** 2026-07-20
**Statut :** design validé (sections 1-7), en attente de relecture spec avant plan d'implémentation.
...
- **Démo simple**, durcissement prod documenté à part (pas construit maintenant).
...
2. `ollama` et `hermes` ne sont que sur `net_internal`. Leur seul usage
   ...
   `ollama` pour **pull le modèle** depuis le registre Ollama (setup), `hermes`
   n'en a **aucun** besoin. Toute destination hors allowlist est refusée →
   exfiltration impossible, même modèle retourné par une injection.
...
`HTTP_PROXY=egress-proxy`, soit via un SearXNG local placé derrière le proxy — le
plus simple qui fonctionne sera retenu. L'invariant (seul le proxy sort,
...
demande » (SKILL.md L48) devient « le RAG récupère les passages pertinents » — la
**logique anti-invention est identique**, seule la source du contexte change
...
2. *« Le prix de la pompe Z ? »* (absente des données) → renvoie
